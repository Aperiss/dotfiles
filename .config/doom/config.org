...
- [[#org-mode][Org Mode]]
  ...
  - [[#zen-mode][Zen Mode]]
  - [[#ttrpg-notes-system][TTRPG Notes System]]
...
** Org Appearance
Modern note-taking appearance for org-mode with improved typography and visual elements.

*** Visual Settings
General org-mode visual enhancements for emphasis, folding, indentation, and links.
...
(after! org
  (setq org-hide-emphasis-markers t        ; Hide markup characters like *bold*
        ...
        org-hidden-keywords '(title)       ; Hide #+title: keyword
        org-link-descriptive t             ; Show only description in links

        ;; Line spacing and wrapping
        fill-column 80)                    ; Max line width for readability

  ;; Enable visual line mode and softer line spacing
...
*** Fonts and Faces
All font and face configuration for org-mode elements.

#+begin_src emacs-lisp
(after! org
  ;; Heading sizes with JetBrains Mono
...
;; Monospaced elements (code, tables, metadata)
(custom-set-faces!
...
*** Bullets and Icons
Org-superstar configuration for heading bullets and list item symbols.
...
;; List bullet demotion behavior
(after! org
  (setq org-list-demote-modify-bullet '(("+" . "-") ("-" . "+") ("*" . "+"))))
#+end_src

*** TODO Keywords and Priorities
TODO state definitions, colors, and priority levels with Catppuccin Mocha theme.
...
(after! org
  ...
  (setq org-todo-keyword-faces
        '(("TODO"      . (:foreground "#a6e3a1" :background "#1e1e2e" :weight bold))
          ("PROJ"      . (:foreground "#89dceb" :background "#1e1e2e" :weight bold))
          ("READ"      . (:foreground "#89b4fa" :background "#1e1e2e" :weight bold))
          ("CHECK"     . (:foreground "#f9e2af" :background "#1e1e2e" :weight bold))
          ("IDEA"      . (:foreground "#f5c2e7" :background "#1e1e2e" :weight bold))
          ("WAITING"   . (:foreground "#fab387" :background "#1e1e2e" :weight bold))
          ("DONE"      . (:foreground "#6c7086" :background "#1e1e2e" :weight bold))
          ("CANCELLED" . (:foreground "#585b70" :background "#1e1e2e" :weight bold))
          ("POSTPONED" . (:foreground "#45475a" :background "#1e1e2e" :weight bold))))
  ...
  (setq org-priority-faces
        '((?A . (:foreground "#f38ba8" :background "#1e1e2e" :weight bold))
          (?B . (:foreground "#fab387" :background "#1e1e2e" :weight bold))
          (?C . (:foreground "#f9e2af" :background "#1e1e2e" :weight bold))
          (?D . (:foreground "#a6e3a1" :background "#1e1e2e" :weight bold))
          (?E . (:foreground "#89b4fa" :background "#1e1e2e" :weight bold))
          (?F . (:foreground "#6c7086" :background "#1e1e2e" :weight bold)))))
#+end_src

*** Source Blocks
Syntax highlighting and indentation settings for code blocks.
...
*** Org Modern
Tag alignment and agenda styling using org-modern.
...
(after! org-modern
  (setq org-auto-align-tags t
        ...
        org-modern-star nil                ; Let org-superstar handle stars
        ;; Agenda styling
...
** Org Roam
Configuration for org-roam note-taking system with fuzzy link resolution and dead link tracking.

*** Basic Configuration
Core org-roam setup with directory, database, and completion settings.
...
Automatically resolve [[Title]] links by searching org-roam database and converting to ID links.
Creates new nodes via capture if they don't exist.
...
If the node doesn't exist, create it via org-roam-capture."
    ;; Only intercept if we're in an org-roam directory and it's a simple fuzzy link
    (if (and (buffer-file-name)
             (string-match-p (expand-file-name org-roam-directory) (buffer-file-name))
             (not (string-match-p ":" s)))  ; Only fuzzy links (no protocol like "id:" or "file:")
        ;; Check if org-roam has a node with this title
        (if-let ((node (org-roam-node-from-title-or-alias s)))
            (progn
              ;; Convert the fuzzy link to an ID link
              (when (not stealth)
                (save-excursion
                  ;; Search backward for the link pattern
                  (when (re-search-backward (concat "\\[\\[" (regexp-quote s) "\\]\\]") (line-beginning-position 0) t)
      ...
      ;; Not an org-roam link or not in org-roam directory - use default behavior
      (funcall orig-fn s avoid-pos stealth)))

  (advice-add 'org-link-search :around #'my/org-roam-resolve-fuzzy-link))
#+end_src

*** Dead Link Detection
Track and display fuzzy links that don't have matching org-roam nodes. Provides both an interactive command to show dead links and a sidebar section in the org-roam buffer.

#+begin_src emacs-lisp
;; Command to show dead links in current buffer
...
;; Dead links section for org-roam buffer sidebar
...
;; Configure org-roam buffer sections
(after! org-roam
...
;; Keybinding for dead links command
(map! :after org-roam
      :map org-mode-map
      :localleader
      :prefix ("r" . "roam")
...
Enhanced search with tag display, backlinks configuration, and link behavior customization.
...
;; Better link following - enable RET to follow links
(setq org-return-follows-link t)
#+end_src

*** Org Roam UI
Interactive graph visualization in browser with keybinding. Provides a live-updating knowledge graph that syncs with Emacs theme and follows node navigation.
...
** Org Keybindings
Custom keybindings for org-mode navigation, editing, and TODO management.

*** Visual Line Navigation
Make j/k move by visual lines for better wrapped text navigation.
...
*** TODO State Cycling
Keybindings for cycling TODO states (C-c C-t also works).
S-left/S-right are used for window resizing, so use localleader instead.

#+begin_src emacs-lisp
(map! :after org
      :map org-mode-map
      :localleader
      :desc "Cycle TODO state forward" "t" (cmd! (org-todo 'right))
      :desc "Cycle TODO state backward" "T" (cmd! (org-todo 'left))
      :desc "Mark as DONE" "x" (cmd! (org-todo "DONE")))
#+end_src

*** Link Bracket Editing
Quickly wrap/unwrap words in org link brackets with M-[ and M-].
...
*** Writeroom Settings
Basic writeroom-mode configuration for width, text scaling, and window behavior.

#+begin_src emacs-lisp
(after! writeroom-mode
  (setq writeroom-restore-window-config nil  ; Don't restore window config
        +zen-text-scale 0                     ; Disable Doom's text scaling
        +zen-mixed-pitch-modes nil            ; Disable Doom's mixed-pitch
        writeroom-width 80))                  ; Set width to 80 characters
#+end_src

*** Font and Layout
Set custom font and fix width calculation for zen mode.
...
*** Appearance Hooks
Customize visual appearance when entering/exiting zen mode.
...
*** Directory Structure
Organized structure for tabletop RPG content (in sort order):
...
*** Auto Zen Mode for TTRPG
Automatically enable zen mode when opening files in the TTRPG org-roam directory.

#+begin_src emacs-lisp
(defun my/auto-zen-for-ttrpg ()
  "Automatically enable zen mode for files in ttrpg org-roam directory."
  (when (and (buffer-file-name)
             (string-match-p (expand-file-name "~/Org/org-roam/ttrpg/") (buffer-file-name))
             (not writeroom-mode))
    (writeroom-mode 1)))

(add-hook 'org-mode-hook #'my/auto-zen-for-ttrpg)
#+end_src
